...
TEST_F(SourceThreadTest, NoOpDestruction)
{
    ...
    auto control = std::make_shared<TestSourceControl>();
    {
        const SourceThread sourceThread(
            backpressureListener, INITIAL<OriginId>, INVALID_QUERY_ID, nullptr, bm, std::make_unique<TestSource>(INITIAL<OriginId>, control));
    }
...
TEST_F(SourceThreadTest, FailureDuringRunning)
{
    ...
    {
        SourceThread sourceThread(backpressureListener, INITIAL<OriginId>, INVALID_QUERY_ID, nullptr, bm, std::make_unique<TestSource>(INITIAL<OriginId>, control));
        verify_non_blocking_start(
            sourceThread,
            [&](const OriginId originId, SourceReturnType::SourceReturnType ret, const std::stop_token&)
            {
